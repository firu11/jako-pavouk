...
func GetUdaje(uzivID uint) (float32, float32, map[string]int, [3]int, [3]int, error) {
	...
	err := DB.QueryRow(`WITH vsechny_aktivity AS ( /* Sjednotíme všechna data uživatele na jednom místě */ SELECT neopravene, delka_textu, cas, datum, chyby_pismenka  FROM dokoncene  WHERE uziv_id = $1 UNION ALL SELECT neopravene, delka_textu, cas, datum, chyby_pismenka  FROM dokoncene_procvic  WHERE uziv_id = $1 ), poslednich_100 AS ( /* Vezmeme pouze 100 nejnovějších záznamů pro výpočet CPM a přesnosti */ SELECT *, (SELECT COALESCE(SUM(value::NUMERIC), 0) FROM jsonb_each_text(chyby_pismenka)) AS opravene FROM vsechny_aktivity ORDER BY datum DESC LIMIT 100 ), default_pokud_prazdno AS ( /* Záloha pro případ, že uživatel nemá vůbec žádné záznamy */ SELECT 0 AS neopravene, 0 AS delka_textu, 0 AS cas, 0 AS opravene, '{}'::jsonb AS chyby_pismenka WHERE NOT EXISTS (SELECT 1 FROM poslednich_100) UNION ALL SELECT neopravene, delka_textu, cas, opravene, chyby_pismenka FROM poslednich_100 ), soucty_pismenek AS ( /* Agregace chyb pismenek z těch posledních 100 záznamů */ SELECT key AS pismeno, SUM(value::NUMERIC) AS soucet FROM default_pokud_prazdno, jsonb_each_text(chyby_pismenka) GROUP BY key ), statistiky_obdobi AS ( /* Výpočet souhrnných statistik (Dnes, 14 dní, Celkem) z celé historie */ SELECT  SUM(cas) FILTER (WHERE datum::date = CURRENT_DATE) AS cas_dnes, SUM(delka_textu) FILTER (WHERE datum::date = CURRENT_DATE) AS napsanych_pismen_dnes, SUM(cas) FILTER (WHERE datum::date > CURRENT_DATE - MAKE_INTERVAL(days => 14)) AS cas_dva_tydny, SUM(delka_textu) FILTER (WHERE datum::date > CURRENT_DATE - MAKE_INTERVAL(days => 14)) AS napsanych_pismen_dva_tydny, SUM(cas) AS cas_celkem, SUM(delka_textu) AS napsanych_pismen_celkem FROM vsechny_aktivity ) SELECT  /* Rychlost a přesnost se počítá jen z CTE 'default_pokud_prazdno' (což je max 100 záznamů) */ GREATEST( ( (SUM(d.delka_textu) - 10 * SUM(d.neopravene)) / GREATEST(SUM(d.cas)::NUMERIC, 1) ) * 60, 0 ) AS rychlost, COALESCE( ( ( SUM(d.delka_textu) - SUM(d.neopravene) - SUM(d.opravene) ) / GREATEST(SUM(d.delka_textu)::NUMERIC, 1) ) * 100, -1 ) AS presnost, /* Písmenka a globální statistiky */ (SELECT COALESCE(jsonb_object_agg(pismeno, soucet), '{}'::jsonb) FROM soucty_pismenek WHERE pismeno IS NOT NULL) AS chyby_pismenka, COALESCE(MAX(s.cas_dnes), 0) AS cas_dnes, COALESCE(MAX(s.cas_dva_tydny), 0) AS cas_dva_tydny, COALESCE(MAX(s.cas_celkem), 0) AS cas_celkem, COALESCE(MAX(s.napsanych_pismen_dnes), 0) AS napsanych_pismen_dnes, COALESCE(MAX(s.napsanych_pismen_dva_tydny), 0) AS napsanych_pismen_dva_tydny, COALESCE(MAX(s.napsanych_pismen_celkem), 0) AS napsanych_pismen_celkem FROM default_pokud_prazdno d CROSS JOIN statistiky_obdobi s; `, uzivID).Scan(&rychlost, &presnost, &chybyPismenkaJsonb, &cas[0], &cas[1], &cas[2], &napsanychPismen[0], &napsanychPismen[1], &napsanychPismen[2])
	if err != nil {
