...
function registr(e: Event) {
    ...
    if (spatnyEmail.value || spatnyHeslo.value || spatnyJmeno.value) {
        if (spatnyJmeno.value && jmeno.value.length > 12) pridatOznameni('Jméno je moc dlouhé.\n(3-12 znaků)');
        else if (spatnyJmeno.value && jmeno.value.length < 3) pridatOznameni('Jméno je moc krátké.\n(3-12 znaků)');
...
function chekujUdaje(jaky: string) {
    if (jaky === 'email' && email.value)
        spatnyEmail.value = !/^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/g.test(email.value);
    else if (jaky === 'heslo' && heslo.value !== undefined)
        spatnyHeslo.value = !/^[ -~]{5,72}$/.test(heslo.value); //ascii 5-72
    else if (jaky === 'jmeno' && jmeno.value !== undefined)
        spatnyJmeno.value = !/^[a-zA-Z0-9ěščřžýáíéůúťňďóĚŠČŘŽÝÁÍÉŮÚŤŇĎÓ_\-+*! ]{3,12}$/.test(jmeno.value); //jmeno 3-12
...
<template>
    ...
    <div v-if="!overeni">
        <form>
            ...
            <input :class="{ 'spatnej-input': spatnyJmeno }" @:input="chekujUdaje('jmeno')" type="text" v-model="jmeno" placeholder="Např: Pan Pavouk" />
    ...
    <div v-else>
        <form id="overeni">
            <h3 style="margin-bottom: 20px">
                Na e-mail: "<i>{{ email }}</i
                >" ti byl zaslán ověřovací kód.
            </h3>
            <h3 class="nadpis">Zadej kód z emailu:</h3>
            <input :class="{ 'spatnej-input': spatnyKod }" @:input="chekujUdaje('kod')" type="text" inputmode="numeric" v-model.trim="kod" placeholder="Např: 12345" />
