        ...
        .then((response) => {
            ...
            if (!menuRef.value.diakritika && !menuRef.value.velkaPismena) {
                for (let i = 0; i < response.data.text.length; i++) {
                    response.data.text[i] = response.data.text[i]
                        .normalize('NFD')
                        .replace(/[\u0300-\u036f]/g, '')
                        .toLocaleLowerCase();
                }
...
<template>
    ...
    <Psani
        ...
        @konec="konecTextu"
        @restart="restart"
        @pise="hideKlavecnice = false"
        @prodlouzit="prodlouzit"
        :text="text"
        :klavesnice="klavesnice"
        :delkaTextu="delkaTextu"
        :hide-klavesnice="hideKlavecnice"
        :nacitam-novej="nacitamNovej"
    ...
    <PsaniMenu
        class="psaniMenu"
        :class="{ hide: konec || !hideKlavecnice }"
        @restart="
            restart();
            psaniRef?.restart();
        "
    ...
    <NastaveniBtn
        v-if="!konec && klavesnice != ''"
        @klik="
            hideKlavecnice = !hideKlavecnice;
            refocus();
            psaniRef?.restart();
        "
    />
