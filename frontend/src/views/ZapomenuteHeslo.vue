...
function zmena() {
    // pokud zacnu znova psat tak zrusim znaceni spatnyho inputu
...
<template>
    ...
    <form v-if="state === 'email'">
        <h3 style="margin-bottom: 20px">
            Na e-mail ti bude zaslán <br />
            ověřovací kód pro obnovení hesla.
        </h3>
        <h3 class="nadpis">Zadej e-mail:</h3>
        <input :class="{ 'spatnej-input': spatnyEmail }" :oninput="zmena" type="text" v-model="email" placeholder="Např: pepa@zdepa.cz" inputmode="email" />
    ...
    <form v-else-if="state === 'kod'">
        <h3 style="margin-bottom: 20px">
            Zkontroluj prosím svou<br />
            e-mailovou schránku.
        </h3>
        <h3 class="nadpis">Kód z e-mailu:</h3>
        <input style="margin-bottom: 20px" :class="{ 'spatnej-input': spatnyKod }" @:input="chekujUdaje('kod')" type="text" inputmode="numeric" v-model.trim="kod" placeholder="Např: 12345" />
        <h3 class="nadpis">Nové heslo:</h3>
        <input :class="{ 'spatnej-input': spatnyHeslo }" @:input="chekujUdaje('heslo')" type="password" v-model="heslo" placeholder="Rozhodně ne 'Pepa123'" />
