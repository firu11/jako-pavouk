...
function capslockCheck(e: KeyboardEvent) {
    // TODO chtelo by to checknout hned po nacteni stranky ale nevim jestli to jde (spíš ne)
...
function nextPismeno() {
    ...
    if (props.text[counterSlov.value].length - 1 === counter.value) {
        // posledni pismeno ve slovu
...
function klik(e: Event) {
    ...
    */
    ...
    if (
        ...
        // tak dáme celé slovo pryč (Ctrl + Backspace zkratka)
...
async function posunoutRadek() {
    ...
    if (aktualniY - lastY > 30) {
        ...
        indexPosunuti++;
        if (indexPosunuti == 1)
            textElem.value!.style.top = '-2.35rem'; // posunuti dolu
...
<template>
    <div id="flex" ref="celyPsani">
        ...
        <span
            ...
            @click="input?.focus()"
            >Klikni sem nebo zmáčkni <span class="klavesa-v-textu">Mezerník</span> !</span
        >
        ...
        <Transition>
            <div v-show="klavesnice != ''">
                <Klavesnice
                    :typ="klavesnice"
                    :aktivniPismeno="aktivniPismeno.znak"
                    :rozmazat="hideKlavesnice || prestalPsat"
                    :cekame="(aktivniPismeno.id == 0 || aktivniPismeno.id == -1) && cass == 0"
                    :full-hide="fullHideKlavesnice"
                />
                <Tooltip
                    v-if="props.resetBtn && (props.text.length != 0 || nacitamePodruhy)"
                    zprava="Restart cvičení <span class='klavesa-v-textu-mensi'>Delete</span>"
                    :sirka="120"
                    :vzdalenost="6"
                    :xOffset="385"
                    :yOffset="-154"
                ...
                <div
                    ...
                    id="hide-btn"
                    @click="
                        fullHideKlavesnice = !fullHideKlavesnice;
                        input?.focus();
                    "
                    :class="{ schovat: route.fullPath == '/prvni-psani' }"
        ...
        <Transition>
            <div
                ...
                v-if="prestalPsat"
                :style="{
                    boxShadow: fullHideKlavesnice ? 'none' : '0px 0px 10px 2px rgba(0, 0, 0, 0.75)',
                    top: route.fullPath == '/prvni-psani' || route.fullPath.split('/')[1] == 'prace' ? '383px' : '418px',
                }"
            >
                <h3>Jsi tam ještě?</h3>
                <p>Přestal/a jsi psát a tak jsme museli cvičení pozastavit. Pokračuj až budeš připravený/á!</p>
            </div>
        </Transition>
    </div>
